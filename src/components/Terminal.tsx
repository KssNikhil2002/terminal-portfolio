...
export function Terminal() {
  ...
  return (
    ...
    >
      {/* Portfolio Link - Top Left - Mobile Responsive */}
      <div className="fixed top-2 left-2 md:top-4 md:left-4 z-10">
        <a
          href="https://kssnikhil.vercel.app/"
          target="_blank"
          rel="noopener noreferrer"
          className={`flex items-center gap-1 md:gap-2 px-2 md:px-3 py-1 md:py-2 rounded-full text-xs font-mono transition-all duration-200 backdrop-blur-sm ${
            theme === 'dark' 
        ...
        >
          ...
          <svg 
            ...
            viewBox="0 0 18 14" 
            fill="none" 
            className="rotate-6 flex-shrink-0 md:w-[18px] md:h-[14px]"
          >
            <path 
          ...
          <span className="hidden sm:inline">check the complete portfolio</span>
          <span className="sm:hidden">portfolio</span>
        </a>
      </div>

      {/* Action Buttons & Theme Toggle - Top Right - Mobile Responsive */}
      <div className="fixed top-2 right-2 md:top-4 md:right-4 z-10 flex gap-1 md:gap-2">
        <a
        ...
        <a
          href="https://drive.google.com/file/d/102_OJVSzwp5X7M2tzc3n9pUrmevXLDSA/view"
          target="_blank"
          rel="noopener noreferrer"
          className={`p-2 md:p-3 rounded-full transition-all duration-200 backdrop-blur-sm ${
            theme === 'dark' 
      ...
      {/* Scrollable Terminal Content - Mobile Responsive */}
      <div className="h-screen overflow-y-auto scroll-smooth">
        <div className={`min-h-screen flex justify-center pt-16 md:pt-20 pb-8 md:pb-20 px-4 md:px-6 ${
          lines.length <= 2 ? 'items-center' : 'items-start'
        }`}>
          <div className="w-full max-w-2xl mx-auto">
            <div 
              ref={terminalRef}
              className="space-y-2 md:space-y-3 font-mono text-xs md:text-sm leading-relaxed transition-all duration-300 ease-in-out"
              onClick={() => {
                const terminalInput = document.getElementById('terminal-input') as HTMLInputElement;
                terminalInput?.focus();
              }}
            >
              {lines.map((line, index) => (
                <TerminalLine 
                  key={`${line.id}-${index}`} 
                  line={line} 
                  isFromStorage={linesFromStorage.has(line.id)}
                />
              ))}
